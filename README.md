...
1. 前往 [Releases](https://github.com/dandelion80231/System-Cleanup-Optimizer/releases/latest) 或 [官网](https://cab.dpdns.org/) 下载最新版 `.exe`（文件名如 `系统清理与优化工具_v1.22.exe`）。
2. 双击运行即可，**无需安装**。所有资源（背景图、图标、SKU 许可令牌、源码包）均已嵌入单文件 exe。
3. 首次使用建议：先创建系统还原点，再进行优化配置。

> 当前版本 `系统清理与优化工具_v1.22.exe`：10,143,356 字节（约 9.67 MB），SHA256：
`78383488f00856909b2e25f47b705e0fe058f567c56ddb7930aa34d0957c3d01`
...
只需分发单个 `系统清理与优化工具_v1.22.exe` 文件（由构建输出 `系统清理与优化工具.exe` 按版本重命名而来；`WebView2Loader.dll` 并排放置，缺失时程序自动从 NuGet 恢复）。所有资源（背景图、图标、SKU 许可令牌、源码包）均已嵌入。
...
最新版本为 **v1.22**（.NET 10 构建，2026-09-19）。各版本完整变更见 [CHANGELOG.md](CHANGELOG.md)。

### v1.22（2026-09-19）

- 全量代码审查修复：安全默认收紧（安装包签名校验默认严格、高危操作补确认弹窗）、文案与真实行为对齐、清理/优化/安全/激活操作更保守（「开始优化」只执行勾选项、还原按优化前原始值、结果统计按真实状态校验、路径转义修复）；修复 WebP 背景图选择不生效并优化切换速度（秒级应用）；创建还原点在 VSS 服务停用时自动重试并给针对性提示；性能与稳定（关窗/切页停止后台轮询、软件页扫描提速、异常弹窗节流、操作互斥防连点）；安全防护页篡改防护区布局重排。详见 CHANGELOG.md v1.22 段。
