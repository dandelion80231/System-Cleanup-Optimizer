...
1. 前往 [Releases](https://github.com/dandelion80231/System-Cleanup-Optimizer/releases/latest) 下载最新版 `.exe`（文件名如 `系统清理与优化工具_v1.06.exe`）。
...
只需分发单个 `系统清理与优化工具_v1.06.exe` 文件（由构建输出 `系统清理与优化工具.exe` 按版本重命名而来）。所有资源（背景图、图标、SKU 许可令牌、源码包）均已嵌入。
