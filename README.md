...
1. 前往 [Releases](https://github.com/dandelion80231/System-Cleanup-Optimizer/releases/latest) 下载最新版 `.exe`（文件名如 `系统清理与优化工具_v1.0.4.exe`）。
...
**核心能力**: 6 大类 34 项常规清理 + 5 项扩展清理（缩略图缓存、D3D 缓存、终端历史、预读取、WinSxS），共 39 个清理项。
...
**核心能力**: 对系统服务进行管理，含一键优化/还原，自动跳过关键系统服务。
...
只需分发单个 `系统清理与优化工具_v1.0.4.exe` 文件（由构建输出 `系统清理与优化工具.exe` 按版本重命名而来）。所有资源（背景图、图标、SKU 许可令牌、源码包）均已嵌入。
...
**Q: 为什么我无法使用某些功能？**
A: 绝大多数修改系统设置的功能（如系统优化、服务优化、Defender 管理）需要管理员权限。请右键点击 exe，选择「以管理员身份运行」。

**Q: 我优化了系统，但想恢复原状怎么办？**
A: 大部分功能都提供「一键还原」或反向操作。例如，在「系统优化」页面点击「还原所有项」，在「服务优化」页面点击「一键还原」。如果不确定，可随时使用「系统工具 → 系统还原点」进行系统恢复。

**Q: 激活工具 (MAS) 是如何工作的，安全吗？**
A: 该功能是一个「启动器」。它会在您的电脑上打开一个提权后的 PowerShell 窗口，并自动执行来自 `https://get.activated.win` 的官方 MAS 脚本。本工具本身不包含、不修改任何激活代码。MAS 是一个开源项目，您可以在其 GitHub 查看源码。

**Q: 软件下载失败怎么办？**
A: 「常用软件」模块的下载链接来自官方源或 Chocolatey 社区，可能受网络环境影响。您可以尝试更换网络环境，或检查软件的下载地址是否仍然有效。对于部分软件，工具也提供了「官方下载页解析」作为兜底策略。

**Q: 如何备份和迁移我的设置？**
A: 将旧电脑 `Config\` 文件夹下的 `autosave.json` 及配置备份文件复制到新电脑 exe 同目录下的 `Config\` 文件夹中，即可恢复所有设置。
