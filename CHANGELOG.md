...
> 相对 v1.22：发布形态升级为「单一智能外壳」——以后下载到的就是一个 exe，首次启动自动补齐运行环境，数据文件夹跟启动入口走；另含一轮全量审查修复与启动引导体验打磨。

### ✨ 新增功能
- **单一智能外壳（单文件交付）**：本版起交付的 exe 为「智能外壳」：首次启动自动检测本机是否缺少 .NET 10 桌面运行时，缺时自动下载官方运行时（约 60MB，国内镜像优先、自动回落官方直链，下载中实时百分比进度）并静默完成安装，装完自动拉起主程序；全程无需手动装运行时、无需复制额外文件到桌面。
- **数据文件夹跟启动入口走**：exe 放在哪个目录（桌面或程序目录），「cpq-tool」数据文件夹就建并读在哪个目录；挪动 exe 后重新运行，数据自动跟回新位置。主程序未拿到入口指引时保持原有行为（exe 同目录）。
- 启动引导「首次启动」对话框：未检测到运行时时的中文双选项（自动安装/官方下载页），安装中进度条滚动提示，装完跳 100%。

### ♻️ 打磨
- 启动引导进度条居中（4/5 宽，不顶满全页），安装阶段显示滚动动画，完成时回 100%。
- 自动安装失败时自动附带安装器日志尾部；并能识别「运行时目录被改名/隐藏（模拟测试状态）」与「安装器判定已装而跳过」两种异常状态，给出可操作的下一步（恢复/真卸载后重试）。
- 「文件夹说明.txt」规则 1 更新为与「数据跟入口」一致的新措辞。
- 一轮全量代码审查（v1.22 tag 之后所有改动）：修正 4 处注释与实现不符、2 处未观察异步 Task、单实例锁改先查后建防误吞新参数；主程序内嵌源码包与本版本源码一致。

### 🔧 发版日修订
- **「导出源码」含外壳源码**：程序内「导出源码」披露包加入 `tools\cpq-shell`（外壳 Program.cs / 构建脚本 / README），导出结果 = 主程序 + 智能外壳的完整源码；主 exe 的 SHA256 常量单独放发版生成文件 `MainExeSha.generated.cs`，内嵌披露包按零值模板 `MainExeSha.zero.cs` 归一化，保证内嵌源码永不成为过期快照（见 tools/cpq-shell/README「SHA 生成文件」节）；安装包已据此重建并同步替换各分发渠道（仅内容增量，无行为变化）。
...
> 相对 v1.20：无新增功能；本版为版本晋升，将 v1.20 之后补入的加固与打磨（路径定位、日志、布局、文件夹说明）并入正式基线，版本号改为 v1.21，后续更新以 v1.21 为准。

### ♻️ 版本晋升
- **无新增功能**：功能内容与 v1.20 一致（Office 部署卡片化 / 多线程下载 / 数据目录合并 / 全驱清理运行时保护等均已在 v1.20 交付）。
- **加固与打磨并入正式基线**（v1.20 之后补入）：
  - 7 处路径定位加固（`Assembly.Location` → `AppPaths.ExeDir` / `Environment.ProcessPath`，单文件 IL3000 修复，消除 WebView2 探针与依赖自动恢复的路径定位失效）；
  - 日志扫尾（CLIXML 噪声）；注册表双根（HKLM/HKCU）读写与删除逻辑重构：按根独立异常处理，单根失败不阻塞另一根；
  - 工具栏五星等宽布局打磨；`文件夹说明.txt` 更新至 v9；
  - `WebView2Loader.dll` 并排分发（缺失时自动从 NuGet 恢复，不嵌入 exe）。
- **版本元数据**：`APP_VERSION` → v1.21，csproj 1.0.20.0 → 1.0.21.0（四处一致），CHANGELOG 新增本段。
