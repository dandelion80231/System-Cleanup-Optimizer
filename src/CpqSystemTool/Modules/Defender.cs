...
namespace CpqSystemTool
{
    ...
    public static class Defender
    {
        ...
        /// <summary>读 Policies 注册表值（无值返回 null）。</summary>
        ...
        public static bool SetRealtime(bool enable, Action<string> log)
        {
            ...
            {
                bool policyOk;
                if (enable) policyOk = RegistryHelper.DeleteKeyTree(Registry.LocalMachine, DEFENDER_RT_POLICY, log);
                else { RegistryHelper.SetDword(Registry.LocalMachine, DEFENDER_RT_POLICY, "DisableRealtimeMonitoring", 1, log); policyOk = true; }
                log(policyOk ? "   [OK] Policies 已" + (enable ? "清理" : "写入") : "   [!!] Policies 失败，安全中心可能仍显示陈旧状态");
            }
        ...
        public static bool SetCloud(bool enable, Action<string> log)
        {
            ...
            try
            {
                ...
                if (enable)
                {
                    policyOk = RegistryHelper.DeleteKeyTree(Registry.LocalMachine, SPYNET_POLICY, log);
                    log(policyOk ? "   [OK] Policies\\Spynet 已删（解除管理员管理）" : "   [!!] Policies\\Spynet 删失败，安全中心仍显示\"由管理员管理+关\"。需 TI 提权。");
                }
                else
                {
        ...
        public static int ClearAllPolicies(Action<string> log)
        {
            ...
            try
            {
                using (var k = Registry.LocalMachine.OpenSubKey(DEFENDER_POLICY, true))
                {
                    if (k == null) { log("   [-] GP 根键不存在: " + DEFENDER_POLICY); }
                    else
                    {
                        int cleared = 0;
                        foreach (var name in new[] { "DisableAntiSpyware", "DisableAntiVirus" })
                            if (k.GetValue(name) != null) { k.DeleteValue(name, false); cleared++; }
            ...
            try
            {
                using (var k = Registry.LocalMachine.OpenSubKey(MDM_POLICY, true))
                {
                    if (k == null) { log("   [-] MDM 键不存在: " + MDM_POLICY); }
                    else
                    {
                        int fixedCount = 0;
                        foreach (var name in MDM_ALLOW_NAMES)
                            if (k.GetValue(name) != null) { k.SetValue(name, 1, RegistryValueKind.DWord); fixedCount++; }
            ...
            foreach (var path in new[] { DEFENDER_RT_POLICY, SPYNET_POLICY, DEFENDER_POLICY })
            {
                ...
                {
                    if (k == null) { log("   [-] 不存在: " + path); continue; }
                }
