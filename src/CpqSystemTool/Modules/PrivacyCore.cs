...
namespace CpqSystemTool
{
    public static class PrivacyCore
    {
        ...
        public static void RemoveSearchFirewallRule(Action<string> log)
        {
            ...
            Exec.RunPowerShell("Remove-NetFirewallRule -DisplayName '阻止Windows搜索联网' -ErrorAction SilentlyContinue", log);
            log("[OK] 防火墙规则已移除");
        }
        ...
        {
            // Start_Layout: 0=关闭(默认), 1=一行, 2=三行(默认), 3=四行
            int val = 0;
            if (rows == 1) val = 1;
