...
namespace CpqSystemTool
{
    ...
    /// 三态开关状态（TweakState）：Off=关 / On=开 / Default=系统默认（不强制写入，交还系统行为）。
    /// </summary>
    ...
    internal static class Tweaks
    {
        ...
        private const string CTRL = @"SYSTEM\CurrentControlSet\Services";
        ...
        private static List<TweakEntry> Build()
        {
            ...
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-no-tab-perf", Name = "禁用标签页性能检测器", Desc = "禁用Edge标签页性能检测器(睡眠标签)", Risk = "low", Enable = log => { RegistryHelper.SetEdgePolicy("PerformanceDetectorEnabled", 0, log); }, Disable = log => { RegistryHelper.DeleteEdgePolicy("PerformanceDetectorEnabled", log); }, State = () => RegistryHelper.GetEdgePolicyState("PerformanceDetectorEnabled", 0) });
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-no-news", Name = "禁用新标签页资讯", Desc = "禁用新选项卡页面上的微软资讯内容", Risk = "low", Enable = log => { RegistryHelper.SetEdgePolicy("NewTabPageContentEnabled", 0, log); }, Disable = log => { RegistryHelper.DeleteEdgePolicy("NewTabPageContentEnabled", log); }, State = () => RegistryHelper.GetEdgePolicyState("NewTabPageContentEnabled", 0) });
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-no-personal-ads", Name = "禁用个性化广告", Desc = "禁用Edge个性化广告和体验", Risk = "low", Enable = log => { RegistryHelper.SetEdgePolicy("PersonalizationReportingEnabled", 0, log); }, Disable = log => { RegistryHelper.DeleteEdgePolicy("PersonalizationReportingEnabled", log); }, State = () => RegistryHelper.GetEdgePolicyState("PersonalizationReportingEnabled", 0) });
            ...
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-bing-ads-suppress", Name = "阻止必应搜索结果中的广告", Desc = "屏蔽 Edge 必应搜索结果的广告（BingAdsSuppression）", Risk = "low", Enable = log => { RegistryHelper.SetEdgePolicy("BingAdsSuppression", 1, log); }, Disable = log => { RegistryHelper.DeleteEdgePolicy("BingAdsSuppression", log); }, State = () => RegistryHelper.GetEdgePolicyState("BingAdsSuppression", 1) });
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-hide-default-topsites", Name = "新标签页隐藏默认热门站点", Desc = "新标签页不再显示默认热门站点（NewTabPageHideDefaultTopSites）", Risk = "low", Enable = log => { RegistryHelper.SetEdgePolicy("NewTabPageHideDefaultTopSites", 1, log); }, Disable = log => { RegistryHelper.DeleteEdgePolicy("NewTabPageHideDefaultTopSites", log); }, State = () => RegistryHelper.GetEdgePolicyState("NewTabPageHideDefaultTopSites", 1) });
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-hide-sidebar", Name = "隐藏 Edge 浏览器边栏", Desc = "隐藏 Edge 右侧边栏（Recommended\\HubsSidebarEnabled）", Risk = "low", Enable = log => { RegistryHelper.SetEdgePolicyRecommended("HubsSidebarEnabled", 0, log); }, Disable = log => { RegistryHelper.DeleteEdgePolicyRecommended("HubsSidebarEnabled", log); }, State = () => RegistryHelper.GetEdgePolicyRecommendedState("HubsSidebarEnabled", 0) });
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-suppress-os-warning", Name = "关闭停止支持旧系统的通知", Desc = "关闭 Edge 停止支持旧系统版本的通知（SuppressUnsupportedOSWarning）", Risk = "low", Enable = log => { RegistryHelper.SetEdgePolicy("SuppressUnsupportedOSWarning", 1, log); }, Disable = log => { RegistryHelper.DeleteEdgePolicy("SuppressUnsupportedOSWarning", log); }, State = () => RegistryHelper.GetEdgePolicyState("SuppressUnsupportedOSWarning", 1) });
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-no-diag-data", Name = "不发送 Edge 诊断数据", Desc = "Edge 不发送任何诊断数据（DiagnosticData）", Risk = "low", Enable = log => { RegistryHelper.SetEdgePolicy("DiagnosticData", 0, log); }, Disable = log => { RegistryHelper.DeleteEdgePolicy("DiagnosticData", log); }, State = () => RegistryHelper.GetEdgePolicyState("DiagnosticData", 0) });
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-disable-insecure-dl-warn", Name = "禁用不安全下载警告", Desc = "⚠ 谨慎：关闭 Edge 对潜在不安全下载的警告（ShowDownloadsInsecureWarningsEnabled）", Risk = "low", Enable = log => { RegistryHelper.SetEdgePolicy("ShowDownloadsInsecureWarningsEnabled", 0, log); }, Disable = log => { RegistryHelper.DeleteEdgePolicy("ShowDownloadsInsecureWarningsEnabled", log); }, State = () => RegistryHelper.GetEdgePolicyState("ShowDownloadsInsecureWarningsEnabled", 0) });
