...
namespace CpqSystemTool
{
    ...
    internal static class Tweaks
    {
        ...
        private static List<TweakEntry> Build()
        {
            ...
            L.Add(new TweakEntry { Group = "外观/资源管理器", Id = "hide-taskbar-search", Name = "隐藏任务栏搜索框", Desc = "隐藏任务栏搜索框", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Search", "SearchboxTaskbarMode", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Search", "SearchboxTaskbarMode", 1, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Search")) return k?.GetValue("SearchboxTaskbarMode") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "外观/资源管理器", Id = "hide-taskview-btn", Name = "隐藏任务视图按钮", Desc = "隐藏任务栏任务视图按钮", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "ShowTaskViewButton", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "ShowTaskViewButton", 1, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced")) return k?.GetValue("ShowTaskViewButton") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "外观/资源管理器", Id = "show-all-tray-icons", Name = "始终在任务栏显示所有图标", Desc = "任务栏显示所有系统托盘图标", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer", "EnableAutoTray", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer", "EnableAutoTray", 1, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer")) return k?.GetValue("EnableAutoTray") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "外观/资源管理器", Id = "boost-foreground", Name = "提高前台程序显示速度", Desc = "提高前台程序CPU优先级响应速度", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Control\PriorityControl", "Win32PrioritySeparation", 38, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Control\PriorityControl", "Win32PrioritySeparation", 26, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SYSTEM\CurrentControlSet\Control\PriorityControl")) return k?.GetValue("Win32PrioritySeparation") is int v && v == 38; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "外观/资源管理器", Id = "disable-window-anim", Name = "不要显示窗口出现和消失动画", Desc = "关闭窗口动画效果", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "TaskbarAnimations", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "TaskbarAnimations", 1, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced")) return k?.GetValue("TaskbarAnimations") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "外观/资源管理器", Id = "explorer-this-pc", Name = "打开资源管理器时显示此电脑", Desc = "默认打开此电脑而非快速访问", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "LaunchTo", 1, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "LaunchTo", 0, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Explorer\Advanced")) return k?.GetValue("LaunchTo") is int v && v == 1; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "性能优化", Id = "no-auto-store-update", Name = "禁止应用商店自动下载安装更新", Desc = "关闭商店自动更新", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\WindowsStore", "AutoDownload", 2, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\WindowsStore", "AutoDownload", 0, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Policies\Microsoft\WindowsStore")) return k?.GetValue("AutoDownload") is int v && v == 2; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "性能优化", Id = "speed-shutdown", Name = "加快关机速度", Desc = "缩短关机等待时间2000ms", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Control", "WaitToKillServiceTimeout", 2000, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Control", "WaitToKillServiceTimeout", 5000, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SYSTEM\CurrentControlSet\Control")) return k?.GetValue("WaitToKillServiceTimeout") is int v && v == 2000; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "性能优化", Id = "disable-kernel-paging", Name = "禁止内核与驱动分页到硬盘", Desc = "禁止系统内核驱动程序分页", Risk = "mid", Enable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management", "DisablePagingExecutive", 1, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management", "DisablePagingExecutive", 0, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management")) return k?.GetValue("DisablePagingExecutive") is int v && v == 1; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "性能优化", Id = "disable-hpet", Name = "禁用高精度事件定时器HPET", Desc = "关闭HPET提高性能", Risk = "mid", Enable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Control\Session Manager\kernel", "DisableHpet", 1, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Control\Session Manager\kernel", "DisableHpet", 0, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SYSTEM\CurrentControlSet\Control\Session Manager\kernel")) return k?.GetValue("DisableHpet") is int v && v == 1; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            ...
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-no-welcome", Name = "Edge不要显示首次运行欢迎页面", Desc = "禁止Edge首次运行欢迎", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Edge", "HideFirstRunExperience", 1, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Edge", "HideFirstRunExperience", 0, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Policies\Microsoft\Edge")) return k?.GetValue("HideFirstRunExperience") is int v && v == 1; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "更新设置", Id = "wu-no-driver", Name = "Windows更新不包括驱动程序", Desc = "排除WU驱动更新", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate", "ExcludeWUDriversInQualityUpdate", 1, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate", "ExcludeWUDriversInQualityUpdate", 0, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate")) return k?.GetValue("ExcludeWUDriversInQualityUpdate") is int v && v == 1; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "隐私设置", Id = "disable-ad-id", Name = "禁用广告标识符", Desc = "关闭广告ID", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\AdvertisingInfo", "Enabled", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Microsoft\Windows\CurrentVersion\AdvertisingInfo", "Enabled", 1, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\AdvertisingInfo")) return k?.GetValue("Enabled") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });

            // === 新增项：对齐 ZyperWin 150+ 项 ===

            L.Add(new TweakEntry { Group = "外观/资源管理器", Id = "show-cloud-files", Name = "快速访问不显示Office.com文件", Desc = "快速访问不显示来自Office.com的文件", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKCU, @"Software\Microsoft\Windows\CurrentVersion\Explorer", "ShowCloudFilesInQuickAccess", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKCU, @"Software\Microsoft\Windows\CurrentVersion\Explorer", "ShowCloudFilesInQuickAccess", 1, log); }, State = () => { try { using (var k = HKCU.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer")) return k?.GetValue("ShowCloudFilesInQuickAccess") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "外观/资源管理器", Id = "show-empty-drives", Name = "显示空的驱动器", Desc = "文件资源管理器显示空的驱动器", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKCU, @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "HideDrivesWithNoMedia", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKCU, @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "HideDrivesWithNoMedia", 1, log); }, State = () => { try { using (var k = HKCU.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced")) return k?.GetValue("HideDrivesWithNoMedia") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "外观/资源管理器", Id = "show-hidden-files", Name = "显示隐藏的文件和文件夹", Desc = "显示隐藏的文件、文件夹和驱动器", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKCU, @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "Hidden", 1, log); }, Disable = log => { RegistryHelper.SetDword(HKCU, @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "Hidden", 2, log); }, State = () => { try { using (var k = HKCU.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced")) return k?.GetValue("Hidden") is int v && v == 1; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "外观/资源管理器", Id = "hide-system-files", Name = "隐藏受保护的系统文件", Desc = "隐藏受保护的操作系统文件(推荐)", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKCU, @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "ShowSuperHidden", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKCU, @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced", "ShowSuperHidden", 1, log); }, State = () => { try { using (var k = HKCU.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced")) return k?.GetValue("ShowSuperHidden") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "性能优化", Id = "hide-start-suggestions", Name = "不允许在开始菜单显示建议", Desc = "关闭开始菜单的应用建议", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Windows\Explorer", "ShowContentInSuggested", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Windows\Explorer", "ShowContentInSuggested", 1, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Policies\Microsoft\Windows\Explorer")) return k?.GetValue("ShowContentInSuggested") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "性能优化", Id = "ceip-disable", Name = "关闭客户体验改善计划(CEIP)", Desc = "关闭Windows客户体验改善计划", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\SQMClient\Windows", "CEIPEnable", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\SQMClient\Windows", "CEIPEnable", 1, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Policies\Microsoft\SQMClient\Windows")) return k?.GetValue("CEIPEnable") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "性能优化", Id = "disable-diagnostic-svc", Name = "禁用诊断服务", Desc = "禁用Diagnostic服务以释放系统资源", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\DPS", "Start", 4, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\DPS", "Start", 3, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SYSTEM\CurrentControlSet\Services\DPS")) return k?.GetValue("Start") is int v && v == 4; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "性能优化", Id = "disable-sysmain-svc", Name = "禁用SysMain服务", Desc = "禁用SysMain(Superfetch)服务。SSD用户可尝试禁用", Risk = "mid", Enable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\SysMain", "Start", 4, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\SysMain", "Start", 3, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SYSTEM\CurrentControlSet\Services\SysMain")) return k?.GetValue("Start") is int v && v == 4; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "性能优化", Id = "disable-wsearch-svc", Name = "禁用Windows Search服务", Desc = "禁用WSearch索引服务。不使用Windows搜索可禁用", Risk = "mid", Enable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\WSearch", "Start", 4, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\WSearch", "Start", 3, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SYSTEM\CurrentControlSet\Services\WSearch")) return k?.GetValue("Start") is int v && v == 4; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "性能优化", Id = "disable-homegroup-svc", Name = "禁用家庭组服务", Desc = "禁用家庭组相关服务", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\HomeGroupListener", "Start", 4, log); RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\HomeGroupProvider", "Start", 4, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\HomeGroupListener", "Start", 3, log); RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\HomeGroupProvider", "Start", 3, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SYSTEM\CurrentControlSet\Services\HomeGroupListener")) return k?.GetValue("Start") is int v && v == 4; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "性能优化", Id = "no-low-disk-warn", Name = "禁用磁盘空间不足警告", Desc = "当磁盘空间不足时不弹出警告通知", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKCU, @"Software\Microsoft\Windows\CurrentVersion\Policies\Explorer", "NoLowDiskSpaceChecks", 1, log); }, Disable = log => { RegistryHelper.SetDword(HKCU, @"Software\Microsoft\Windows\CurrentVersion\Policies\Explorer", "NoLowDiskSpaceChecks", 0, log); }, State = () => { try { using (var k = HKCU.OpenSubKey(@"Software\Microsoft\Windows\CurrentVersion\Policies\Explorer")) return k?.GetValue("NoLowDiskSpaceChecks") is int v && v == 1; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            ...
            L.Add(new TweakEntry { Group = "安全设置", Id = "disable-smart-app-control", Name = "关闭智能应用控制", Desc = "关闭智能应用控制(Smart App Control)", Risk = "mid", Enable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Control\CI\Policy", "VerifiedAndReputablePolicyState", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Control\CI\Policy", "VerifiedAndReputablePolicyState", 1, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SYSTEM\CurrentControlSet\Control\CI\Policy")) return k?.GetValue("VerifiedAndReputablePolicyState") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-no-tab-perf", Name = "禁用标签页性能检测器", Desc = "禁用Edge标签页性能检测器(睡眠标签)", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Edge", "PerformanceDetectorEnabled", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Edge", "PerformanceDetectorEnabled", 1, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Policies\Microsoft\Edge")) return k?.GetValue("PerformanceDetectorEnabled") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-no-news", Name = "禁用新标签页资讯", Desc = "禁用新选项卡页面上的微软资讯内容", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Edge", "NewTabPageContentEnabled", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Edge", "NewTabPageContentEnabled", 1, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Policies\Microsoft\Edge")) return k?.GetValue("NewTabPageContentEnabled") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "Edge优化", Id = "edge-no-personal-ads", Name = "禁用个性化广告", Desc = "禁用Edge个性化广告和体验", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Edge", "PersonalizationReportingEnabled", 0, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Edge", "PersonalizationReportingEnabled", 1, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Policies\Microsoft\Edge")) return k?.GetValue("PersonalizationReportingEnabled") is int v && v == 0; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            ...
            L.Add(new TweakEntry { Group = "系统设置", Id = "disable-system-restore", Name = "关闭系统还原", Desc = "禁用系统还原功能以释放磁盘空间", Risk = "mid", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Windows NT\SystemRestore", "DisableSR", 1, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Windows NT\SystemRestore", "DisableSR", 0, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Policies\Microsoft\Windows NT\SystemRestore")) return k?.GetValue("DisableSR") is int v && v == 1; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            L.Add(new TweakEntry { Group = "更新设置", Id = "wu-no-reboot-logged", Name = "更新挂起不自动重启", Desc = "更新挂起时若有用户登录则不自动重启计算机", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU", "NoAutoRebootWithLoggedOnUsers", 1, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU", "NoAutoRebootWithLoggedOnUsers", 0, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SOFTWARE\Policies\Microsoft\Windows\WindowsUpdate\AU")) return k?.GetValue("NoAutoRebootWithLoggedOnUsers") is int v && v == 1; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
            ...
            L.Add(new TweakEntry { Group = "隐私设置", Id = "no-sms-router", Name = "禁用SMS路由器服务", Desc = "禁用SMS路由器服务以保护隐私", Risk = "low", Enable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\SmsRouter", "Start", 4, log); }, Disable = log => { RegistryHelper.SetDword(HKLM, @"SYSTEM\CurrentControlSet\Services\SmsRouter", "Start", 3, log); }, State = () => { try { using (var k = HKLM.OpenSubKey(@"SYSTEM\CurrentControlSet\Services\SmsRouter")) return k?.GetValue("Start") is int v && v == 4; } catch (Exception caughtEx) { System.Diagnostics.Debug.WriteLine("[CpqSystemTool] 异常(已忽略): " + caughtEx.Message);  return false; } } });
