...
namespace CpqSystemTool
{
    ...
    public static class EditionMap
    {
        ...
        public static string ToChinese(string english)
        {
            ...
            // 去掉 "Single Language" 后缀再试（原 ChineseEditionName 的兼容逻辑；权威源键不含该形式，保留以防万一）
            if (english.EndsWith("Single Language", StringComparison.OrdinalIgnoreCase))
            {
                string alt = english.Substring(0, english.Length - "Single Language".Length).Trim() + " 单语言版";
                if (EnglishToChinese.TryGetValue(alt, out cn)) return cn;
            }
