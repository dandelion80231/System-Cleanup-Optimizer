...
namespace CpqSystemTool
{
    ...
    internal static class Cleanup
    {
        ...
        // 不弹确认框、不打断自动化清理流程。
        private static string[] _protectedSubtreeRoots;   // 连同其子目录一并保护
        private static string[] _protectedSelfRoots;      // 仅保护该目录本身

        private static void EnsureProtectedRoots()
        {
            if (_protectedSubtreeRoots != null) return;
        ...
        internal static void CleanWholeDriveCaches(bool tier2, string prefix, Action<string> log)
        {
            ...
            foreach (var g in paths.GroupBy(p => Path.GetFileName(p)).OrderBy(g => g.Key, StringComparer.OrdinalIgnoreCase))
            {
                ...
                if (ok > 0) line += " [OK]";
                else if (skip == 0) line += " [SKIP] 受保护路径，未删除";
                log(line);
            }
        }
