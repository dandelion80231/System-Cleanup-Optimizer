...
namespace CpqSystemTool
{
    ...
    internal static class ServiceOptimizer
    {
        ...
        public static void Apply(ServiceEntry e, bool enable, Action<string> log)
        {
            ...
            // 修正：原先 SetService 不返回成败、Apply 无条件打印 [OK]，sc 配置失败也显示成功
            if (SetService(e.Name, enable, log)) log("  [OK]");
        ...
        private static bool SetService(string name, bool enable, Action<string> log)
        {
            ...
            if (rc != 0)
            {
                log("  [FAIL] sc config 失败（退出码 " + rc + "）");
                return false;
            }
            // 启/停只是让配置立即生效：服务本就处于目标状态时 sc 会返回非零（如 1062「服务尚未启动」），
            // 属正常情况，只提示不计入失败。
            int rc2 = enable ? Exec.RunCmd(new[] { "sc", "start", name }, log)
                             : Exec.RunCmd(new[] { "sc", "stop", name }, log);
            if (rc2 != 0)
                log("  [!] " + (enable ? "sc start" : "sc stop") + " 返回 " + rc2 + "（服务可能已处于目标状态，启动类型已生效）");
            return true;
        }
    }
}
